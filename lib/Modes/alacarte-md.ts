// This file is based on codemirror/markdown/markdown.js
// added some tokens and made it configurable

import * as CodeMirror from 'codemirror'
import 'codemirror/mode/xml/xml'
import 'codemirror/mode/meta'
import { ModeConfigurarion, TokenTypes } from './types'

const hrRE = /^([*\-_])(?:\s*\1){2,}\s*$/
const listRE = /^(?:[*\-+]|^[0-9]+([.)]))\s+/
const taskListRE = /^\[(x| )\](?=\s)/i // Must follow listRE
let atxHeaderRE = /^(#+)(?: |$)/ // Might be re-substituted if modeCfg.allowAtxHeaderWithoutSpace is true
...
const defaultModeConfiguration: Required<ModeConfigurarion> = {
  // Should Allow space between # and heading text?
...
const defaultTokenTypes: Required<TokenTypes> = {
  header: 'header',
  code: 'comment',
  quote: 'quote',
  list1: 'variable-2',
...
): CodeMirror.Mode<unknown> | null {
  const mode = CodeMirror.getMode(cmCfg, name)
  return mode.name === 'null' ? null : mode
}

CodeMirror.defineMode('alacarte-md', function(
  cmCfg: CodeMirror.EditorConfiguration,
  _modeCfg: ModeConfigurarion
) {
  const htmlMode = CodeMirror.getMode(cmCfg, 'text/html')
  const htmlModeMissing = htmlMode.name == 'null'

  const modeCfg = Object.assign(defaultModeConfiguration, _modeCfg)
  const tokenTypes = Object.assign(
    defaultTokenTypes,
    modeCfg.tokenTypeOverrides
  )

  if (modeCfg.allowAtxHeaderWithoutSpace) {
    atxHeaderRE = /^(#+)/
  }
  ...
  function blockNormal(stream, state) {
    ...
    if (
      ...
      // try switching mode
      state.localMode =
        modeCfg.fencedCodeBlockHighlighting && getMode(match[2], cmCfg)
      if (state.localMode)
...
CodeMirror.defineMIME('text/alacarte-md', 'alacarte-md')
CodeMirror.defineMIME('text/x-alacarte-md', 'alacarte-md')
